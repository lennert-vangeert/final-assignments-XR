...
const MenuPlane = () => {
  ...
  // Animation variables
  const bobbingSpeed = 0.5; // Speed of bobbing
  const bobbingHeight = 0.2; // Height of bobbing
  ...
  useFrame(({ clock }) => {
    if (planeGroup.current) {
      ...
      // Bobbing effect
      planeGroup.current.position.y = 113 + Math.sin(elapsedTime * bobbingSpeed) * bobbingHeight;

      // Subtle rotation effect
      planeGroup.current.rotation.z = Math.sin(elapsedTime * rotationSpeed) * 0.05; // Small tilting
      planeGroup.current.rotation.x = Math.cos(elapsedTime * rotationSpeed) * 0.02; // Slight nodding
    }
  });
